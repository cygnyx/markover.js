...
side too. Interpreters like [nodejs](http://nodejs.org) and
...
code, span, b, i, em { display: inline; }
...
Title and tag are each in a single id and only appear once in the document.
...
```css#+stylesheet
@media screen {
  .lang-js { background-color: ivory; }
  .lang-md { background-color: lightyellow; }
  .lang-html { background-color: floralwhite; }
  .lang-json { background-color: honeydew; }
  .lang-css { background-color: cornsilk; }
  .lang-sh { background-color: lemonchiffon; }
}
```
...
Create some short cuts for setting up the document.

```js#weaveJSON
  var un = this.unescape
  var f = p.field
```
...
```js#weaveJSON
  var title = un(f["title"] || "Weave Output")
  var tag = un(f["tag"] || "from weaving")
  var stylesheet = un(f["stylesheet"] || "")

  var headline = un(f["head"] || "<!DOCTYPE html><html><head>")
  var headtitle = '<title>' + title + ' - ' + tag + '</title>\n'
  var transitionline = un(f["transition"] || "</head><body>")
  var titleline = '<div id="title">' + title + '</div>'
  var tagline = '<div id="tag">' + tag + '</div>'
  var styleline = stylesheet == '' ? '' : '<style>' + stylesheet + '</style>'
  var tailline = un(f["tail"] || "</body></html>") + '\n'
...
```js#weaveJSON
  return (headline + headtitle + styleline + transitionline +
          '<div id="content">' +
